...
struct stream_read_await : system_op {
    ...
    std::coroutine_handle<>
        ...
        auto err = uv::read_start(self->as<uv_stream_t>(), on_alloc, on_read);
        if(err.has_error()) {
            self->error_code = err;
            return waiting;
        }
...
struct stream_write_await : system_op {
    ...
    std::coroutine_handle<>
        ...
        auto err =
            uv::write(req, self->as<uv_stream_t>(), std::span<const uv_buf_t>{&buf, 1}, on_write);
        if(err.has_error()) {
            error_code = err;
            self->writer = nullptr;
            return waiting;
...
handle_type guess_handle(int fd) {
    return to_handle_type(uv::guess_handle(fd));
}
...
error stream::set_blocking(bool enabled) {
    ...
    auto err = uv::stream_set_blocking(self->as<uv_stream_t>(), enabled);
    if(err.has_error()) {
        return err;
    }
...
result<pipe> pipe::open(int fd, pipe::options opts, event_loop& loop) {
    ...
    auto err = uv::pipe_open(handle, fd);
    if(err.has_error()) {
...
static error start_pipe_listen(pipe::acceptor& acc,
    ...
    return uv::listen(handle, opts.backlog, pipe_accept_await::on_connection_cb);
}
...
static error start_tcp_listen(tcp_socket::acceptor& acc,
    ...
    return uv::listen(handle, opts.backlog, tcp_accept_await::on_connection_cb);
}
...
error console::set_mode(mode value) {
    ...
    auto err = uv::tty_set_mode(self->as<uv_tty_t>(), uv_mode);
    if(err.has_error()) {
        return err;
    }
