...
result<process_info> process::query_info(int pid) {
    process_info info{};
    info.pid = pid;
    ...
    // Helper: skip whitespace, then advance past the next space-delimited token.
    ...
    // Helper: parse the next space-delimited unsigned long from [cur, end).
    ...
    // Memory from /proc/<pid>/statm: "vsize_pages rss_pages ..." (all in pages).
    {
        ...
        info.vsize = vsize_pages * page_size;
    ...
    // Format: "pid (comm) state field4 ... field14(utime) field15(stime) ..."
    {
        ...
        if(content) {
            // Find last ')' to skip past the comm field (may contain spaces).
            ...
            if(rp != std::string::npos && rp + 1 < content->size()) {
                ...
                // Skip state (1 char token) then 10 numeric fields (4..13).
                ...
                info.utime_us = utime_ticks * 1'000'000ULL / static_cast<std::uint64_t>(hz);
                info.stime_us = stime_ticks * 1'000'000ULL / static_cast<std::uint64_t>(hz);
            }
        }
    }
    ...
    info.vsize = pti.pti_virtual_size;
    // pti_total_user / pti_total_system are in nanoseconds on macOS.
    info.utime_us = pti.pti_total_user / 1000;
    ...
    if(GetProcessMemoryInfo(h, &pmc, sizeof(pmc))) {
        ...
        // PagefileUsage is a reasonable proxy for committed virtual memory.
    ...
    if(GetProcessTimes(h, &creation, &exit_time, &kernel_time, &user_time)) {
        ...
        info.stime_us = to_us(kernel_time);
    }
