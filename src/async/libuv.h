...
#if defined(NDEBUG)
#if defined(_MSC_VER)
#define EVENTIDE_UV_ALWAYS_INLINE __forceinline
#elif defined(__GNUC__) || defined(__clang__)
#define EVENTIDE_UV_ALWAYS_INLINE inline __attribute__((always_inline))
#else
#define EVENTIDE_UV_ALWAYS_INLINE inline
#endif
#else
#define EVENTIDE_UV_ALWAYS_INLINE inline
#endif
...
EVENTIDE_UV_ALWAYS_INLINE uv_handle_t* as_handle(H& handle) noexcept {
...
template <handle_like H>
EVENTIDE_UV_ALWAYS_INLINE const uv_handle_t* as_handle(const H& handle) noexcept {
    if constexpr(std::same_as<bare_t<H>, uv_handle_t>) {
        return &handle;
    } else {
...
EVENTIDE_UV_ALWAYS_INLINE uv_stream_t* as_stream(S& stream) noexcept {
...
template <stream_like S>
EVENTIDE_UV_ALWAYS_INLINE const uv_stream_t* as_stream(const S& stream) noexcept {
    if constexpr(std::same_as<bare_t<S>, uv_stream_t>) {
        return &stream;
    } else {
...
EVENTIDE_UV_ALWAYS_INLINE error status_to_error(int rc) noexcept {
    return rc == 0 ? error{} : error(rc);
}

template <handle_like H>
EVENTIDE_UV_ALWAYS_INLINE bool is_active(const H& handle) noexcept {
    return ::uv_is_active(as_handle(handle)) != 0;
}

template <stream_like S>
EVENTIDE_UV_ALWAYS_INLINE bool is_readable(const S& stream) noexcept {
    return ::uv_is_readable(as_stream(stream)) != 0;
}

template <stream_like S>
EVENTIDE_UV_ALWAYS_INLINE bool is_writable(const S& stream) noexcept {
    return ::uv_is_writable(as_stream(stream)) != 0;
}

template <handle_like H>
EVENTIDE_UV_ALWAYS_INLINE bool is_closing(const H& handle) noexcept {
    return ::uv_is_closing(as_handle(handle)) != 0;
}

template <handle_like H>
EVENTIDE_UV_ALWAYS_INLINE void ref(H& handle) noexcept {
    ::uv_ref(as_handle(handle));
}

template <handle_like H>
EVENTIDE_UV_ALWAYS_INLINE void unref(H& handle) noexcept {
    ::uv_unref(as_handle(handle));
}

template <handle_like H>
EVENTIDE_UV_ALWAYS_INLINE void close(H& handle, uv_close_cb cb = nullptr) noexcept {
    ::uv_close(as_handle(handle), cb);
}

template <req_like R>
EVENTIDE_UV_ALWAYS_INLINE error cancel(R& req) noexcept {
...
EVENTIDE_UV_ALWAYS_INLINE error loop_init(uv_loop_t& loop) noexcept {
    // Errors: UV_ENOMEM and platform init failures.
    return status_to_error(::uv_loop_init(&loop));
}

EVENTIDE_UV_ALWAYS_INLINE error loop_close(uv_loop_t& loop) noexcept {
    // Errors: UV_EBUSY when active handles/requests remain.
    return status_to_error(::uv_loop_close(&loop));
}

EVENTIDE_UV_ALWAYS_INLINE int run(uv_loop_t& loop, uv_run_mode mode) noexcept {
    return ::uv_run(&loop, mode);
}

EVENTIDE_UV_ALWAYS_INLINE void stop(uv_loop_t& loop) noexcept {
    ::uv_stop(&loop);
}

EVENTIDE_UV_ALWAYS_INLINE void walk(uv_loop_t& loop, uv_walk_cb cb, void* arg) noexcept {
    assert(cb != nullptr && "uv::walk requires non-null callback");
    ::uv_walk(&loop, cb, arg);
}

EVENTIDE_UV_ALWAYS_INLINE void idle_init(uv_loop_t& loop, uv_idle_t& handle) noexcept {
    int rc = ::uv_idle_init(&loop, &handle);
    assert(rc == 0 && "uv::idle_init failed");
}

EVENTIDE_UV_ALWAYS_INLINE void idle_start(uv_idle_t& handle, uv_idle_cb cb) noexcept {
    assert(cb != nullptr && "uv::idle_start requires non-null callback");
    int rc = ::uv_idle_start(&handle, cb);
    assert(rc == 0 && "uv::idle_start failed");
}

EVENTIDE_UV_ALWAYS_INLINE void idle_stop(uv_idle_t& handle) noexcept {
    int rc = ::uv_idle_stop(&handle);
    assert(rc == 0 && "uv::idle_stop failed");
}

EVENTIDE_UV_ALWAYS_INLINE void prepare_init(uv_loop_t& loop, uv_prepare_t& handle) noexcept {
    int rc = ::uv_prepare_init(&loop, &handle);
    assert(rc == 0 && "uv::prepare_init failed");
}

EVENTIDE_UV_ALWAYS_INLINE void prepare_start(uv_prepare_t& handle, uv_prepare_cb cb) noexcept {
...
EVENTIDE_UV_ALWAYS_INLINE void prepare_stop(uv_prepare_t& handle) noexcept {
    int rc = ::uv_prepare_stop(&handle);
    assert(rc == 0 && "uv::prepare_stop failed");
}

EVENTIDE_UV_ALWAYS_INLINE void check_init(uv_loop_t& loop, uv_check_t& handle) noexcept {
    int rc = ::uv_check_init(&loop, &handle);
    assert(rc == 0 && "uv::check_init failed");
}

EVENTIDE_UV_ALWAYS_INLINE void check_start(uv_check_t& handle, uv_check_cb cb) noexcept {
...
EVENTIDE_UV_ALWAYS_INLINE void check_stop(uv_check_t& handle) noexcept {
    int rc = ::uv_check_stop(&handle);
    assert(rc == 0 && "uv::check_stop failed");
}

EVENTIDE_UV_ALWAYS_INLINE void timer_init(uv_loop_t& loop, uv_timer_t& handle) noexcept {
    int rc = ::uv_timer_init(&loop, &handle);
    assert(rc == 0 && "uv::timer_init failed");
}

EVENTIDE_UV_ALWAYS_INLINE
...
EVENTIDE_UV_ALWAYS_INLINE void timer_stop(uv_timer_t& handle) noexcept {
    int rc = ::uv_timer_stop(&handle);
    assert(rc == 0 && "uv::timer_stop failed");
}

EVENTIDE_UV_ALWAYS_INLINE error signal_init(uv_loop_t& loop, uv_signal_t& handle) noexcept {
    // Errors: backend signal-loop init failures.
    return status_to_error(::uv_signal_init(&loop, &handle));
}

EVENTIDE_UV_ALWAYS_INLINE error signal_start(uv_signal_t& handle,
                                             uv_signal_cb cb,
                                             int signum) noexcept {
...
EVENTIDE_UV_ALWAYS_INLINE error signal_stop(uv_signal_t& handle) noexcept {
    return status_to_error(::uv_signal_stop(&handle));
}

EVENTIDE_UV_ALWAYS_INLINE error queue_work(uv_loop_t& loop,
                                           uv_work_t& req,
                                           uv_work_cb work_cb,
...
EVENTIDE_UV_ALWAYS_INLINE error fs_event_init(uv_loop_t& loop, uv_fs_event_t& handle) noexcept {
...
EVENTIDE_UV_ALWAYS_INLINE error fs_event_stop(uv_fs_event_t& handle) noexcept {
    return status_to_error(::uv_fs_event_stop(&handle));
}

template <stream_like S>
EVENTIDE_UV_ALWAYS_INLINE error read_start(S& stream,
...
template <stream_like S>
EVENTIDE_UV_ALWAYS_INLINE void read_stop(S& stream) noexcept {
    int rc = ::uv_read_stop(as_stream(stream));
    assert(rc == 0 && "uv::read_stop failed");
}

template <stream_like S>
EVENTIDE_UV_ALWAYS_INLINE error
...
EVENTIDE_UV_ALWAYS_INLINE error accept(Server& server, Client& client) noexcept {
    return status_to_error(::uv_accept(as_stream(server), as_stream(client)));
}

template <stream_like S>
EVENTIDE_UV_ALWAYS_INLINE error listen(S& stream, int backlog, uv_connection_cb cb) noexcept {
    assert(cb != nullptr && "uv::listen requires non-null callback");
    return status_to_error(::uv_listen(as_stream(stream), backlog, cb));
}

EVENTIDE_UV_ALWAYS_INLINE error pipe_init(uv_loop_t& loop, uv_pipe_t& handle, int ipc) noexcept {
    return status_to_error(::uv_pipe_init(&loop, &handle, ipc));
}

EVENTIDE_UV_ALWAYS_INLINE error pipe_open(uv_pipe_t& handle, uv_file fd) noexcept {
    return status_to_error(::uv_pipe_open(&handle, fd));
}

EVENTIDE_UV_ALWAYS_INLINE error pipe_bind2(uv_pipe_t& handle,
                                           const char* name,
                                           std::size_t namelen,
                                           unsigned flags) noexcept {
...
EVENTIDE_UV_ALWAYS_INLINE error tcp_init(uv_loop_t& loop, uv_tcp_t& handle) noexcept {
    return status_to_error(::uv_tcp_init(&loop, &handle));
}

EVENTIDE_UV_ALWAYS_INLINE error tcp_open(uv_tcp_t& handle, uv_os_sock_t sock) noexcept {
    return status_to_error(::uv_tcp_open(&handle, sock));
}

EVENTIDE_UV_ALWAYS_INLINE error tcp_bind(uv_tcp_t& handle,
                                         const sockaddr* addr,
                                         unsigned flags) noexcept {
...
EVENTIDE_UV_ALWAYS_INLINE error tcp_connect(uv_connect_t& req,
                                            uv_tcp_t& handle,
                                            const sockaddr* addr,
                                            uv_connect_cb cb) noexcept {
...
EVENTIDE_UV_ALWAYS_INLINE uv_handle_type guess_handle(uv_file file) noexcept {
    return ::uv_guess_handle(file);
}

template <stream_like S>
EVENTIDE_UV_ALWAYS_INLINE result<std::size_t> try_write(S& stream,
...
template <stream_like S>
EVENTIDE_UV_ALWAYS_INLINE error stream_set_blocking(S& stream, bool enabled) noexcept {
    return status_to_error(::uv_stream_set_blocking(as_stream(stream), enabled ? 1 : 0));
}

EVENTIDE_UV_ALWAYS_INLINE error tty_init(uv_loop_t& loop,
                                         uv_tty_t& handle,
                                         uv_file fd,
...
EVENTIDE_UV_ALWAYS_INLINE error tty_set_mode(uv_tty_t& handle, uv_tty_mode_t mode) noexcept {
    return status_to_error(::uv_tty_set_mode(&handle, mode));
}

EVENTIDE_UV_ALWAYS_INLINE error tty_reset_mode() noexcept {
    return status_to_error(::uv_tty_reset_mode());
}

EVENTIDE_UV_ALWAYS_INLINE result<tty_winsize> tty_get_winsize(uv_tty_t& handle) noexcept {
    tty_winsize out{};
...
EVENTIDE_UV_ALWAYS_INLINE void tty_set_vterm_state(uv_tty_vtermstate_t state) noexcept {
    ::uv_tty_set_vterm_state(state);
}

EVENTIDE_UV_ALWAYS_INLINE result<uv_tty_vtermstate_t> tty_get_vterm_state() noexcept {
...
EVENTIDE_UV_ALWAYS_INLINE error udp_init(uv_loop_t& loop, uv_udp_t& handle) noexcept {
...
EVENTIDE_UV_ALWAYS_INLINE error udp_open(uv_udp_t& handle, uv_os_sock_t sock) noexcept {
...
EVENTIDE_UV_ALWAYS_INLINE error udp_connect(uv_udp_t& handle, const sockaddr* addr) noexcept {
    return status_to_error(::uv_udp_connect(&handle, addr));
}

EVENTIDE_UV_ALWAYS_INLINE error udp_recv_start(uv_udp_t& handle,
                                               uv_alloc_cb alloc_cb,
...
EVENTIDE_UV_ALWAYS_INLINE void udp_recv_stop(uv_udp_t& handle) noexcept {
...
EVENTIDE_UV_ALWAYS_INLINE error udp_set_membership(uv_udp_t& handle,
...
EVENTIDE_UV_ALWAYS_INLINE error udp_set_source_membership(uv_udp_t& handle,
                                                          const char* multicast_addr,
                                                          const char* interface_addr,
...
EVENTIDE_UV_ALWAYS_INLINE error udp_set_multicast_loop(uv_udp_t& handle, bool on) noexcept {
    return status_to_error(::uv_udp_set_multicast_loop(&handle, on ? 1 : 0));
}

EVENTIDE_UV_ALWAYS_INLINE error udp_set_multicast_ttl(uv_udp_t& handle, int ttl) noexcept {
...
EVENTIDE_UV_ALWAYS_INLINE error udp_set_broadcast(uv_udp_t& handle, bool on) noexcept {
    return status_to_error(::uv_udp_set_broadcast(&handle, on ? 1 : 0));
}

EVENTIDE_UV_ALWAYS_INLINE error udp_set_ttl(uv_udp_t& handle, int ttl) noexcept {
    return status_to_error(::uv_udp_set_ttl(&handle, ttl));
}

EVENTIDE_UV_ALWAYS_INLINE bool udp_using_recvmmsg(const uv_udp_t& handle) noexcept {
    return ::uv_udp_using_recvmmsg(&handle) != 0;
}

EVENTIDE_UV_ALWAYS_INLINE std::size_t udp_get_send_queue_size(const uv_udp_t& handle) noexcept {
    return ::uv_udp_get_send_queue_size(&handle);
}

EVENTIDE_UV_ALWAYS_INLINE std::size_t udp_get_send_queue_count(const uv_udp_t& handle) noexcept {
    return ::uv_udp_get_send_queue_count(&handle);
}

EVENTIDE_UV_ALWAYS_INLINE error spawn(uv_loop_t& loop,
...
EVENTIDE_UV_ALWAYS_INLINE error process_kill(uv_process_t& process, int signum) noexcept {
    return status_to_error(::uv_process_kill(&process, signum));
}

EVENTIDE_UV_ALWAYS_INLINE uv_buf_t buf_init(char* base, unsigned int len) noexcept {
    return ::uv_buf_init(base, len);
}

EVENTIDE_UV_ALWAYS_INLINE error ip4_addr(const char* ip, int port, sockaddr_in& out) noexcept {
    assert(ip != nullptr && "uv::ip4_addr requires non-null ip");
    return status_to_error(::uv_ip4_addr(ip, port, &out));
}

EVENTIDE_UV_ALWAYS_INLINE error ip6_addr(const char* ip, int port, sockaddr_in6& out) noexcept {
    assert(ip != nullptr && "uv::ip6_addr requires non-null ip");
    return status_to_error(::uv_ip6_addr(ip, port, &out));
}

EVENTIDE_UV_ALWAYS_INLINE error ip4_name(const sockaddr_in& src,
                                         char* dst,
...
EVENTIDE_UV_ALWAYS_INLINE error ip6_name(const sockaddr_in6& src,
                                         char* dst,
                                         std::size_t size) noexcept {
...
EVENTIDE_UV_ALWAYS_INLINE std::string_view strerror(int code) noexcept {
    const char* msg = ::uv_strerror(code);
    return msg == nullptr ? std::string_view{} : std::string_view(msg);
}

EVENTIDE_UV_ALWAYS_INLINE void fs_req_cleanup(uv_fs_t& req) noexcept {
...
EVENTIDE_UV_ALWAYS_INLINE error fs_copyfile(uv_loop_t& loop,
                                            uv_fs_t& req,
                                            const char* path,
                                            const char* new_path,
                                            int flags,
...
EVENTIDE_UV_ALWAYS_INLINE error fs_scandir_next(uv_fs_t& req, uv_dirent_t& ent) noexcept {
...
EVENTIDE_UV_ALWAYS_INLINE error fs_fstat(uv_loop_t& loop,
                                         uv_fs_t& req,
                                         uv_file file,
                                         uv_fs_cb cb) noexcept {
...
EVENTIDE_UV_ALWAYS_INLINE error fs_rename(uv_loop_t& loop,
                                          uv_fs_t& req,
                                          const char* path,
                                          const char* new_path,
                                          uv_fs_cb cb) noexcept {
...
EVENTIDE_UV_ALWAYS_INLINE error fs_sendfile(uv_loop_t& loop,
                                            uv_fs_t& req,
                                            uv_file out_file,
                                            uv_file in_file,
                                            int64_t in_offset,
...
EVENTIDE_UV_ALWAYS_INLINE error fs_utime(uv_loop_t& loop,
                                         uv_fs_t& req,
                                         const char* path,
                                         double atime,
                                         double mtime,
...
EVENTIDE_UV_ALWAYS_INLINE error fs_futime(uv_loop_t& loop,
                                          uv_fs_t& req,
                                          uv_file file,
                                          double atime,
                                          double mtime,
...
EVENTIDE_UV_ALWAYS_INLINE error fs_lutime(uv_loop_t& loop,
                                          uv_fs_t& req,
                                          const char* path,
                                          double atime,
                                          double mtime,
...
EVENTIDE_UV_ALWAYS_INLINE error fs_link(uv_loop_t& loop,
                                        uv_fs_t& req,
                                        const char* path,
                                        const char* new_path,
                                        uv_fs_cb cb) noexcept {
...
public:
    ...
    bool is_active() const noexcept {
        ...
        return uv::is_active(*handle_ptr());
    }
...
#undef EVENTIDE_UV_ALWAYS_INLINE
