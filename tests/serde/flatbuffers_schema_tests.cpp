...
TEST_CASE(trivial_struct_maps_to_schema_struct) {
    ASSERT_TRUE((eventide::serde::flatbuffers::schema::is_schema_struct_v<point2d>));
    ASSERT_FALSE((eventide::serde::flatbuffers::schema::is_schema_struct_v<payload>));
}

TEST_CASE(map_field_emits_binary_search_entry_table) {
    const auto schema = eventide::serde::flatbuffers::schema::render<payload>();
    const auto point_name = eventide::serde::flatbuffers::schema::type_identifier<point2d>();
    const auto payload_name = eventide::serde::flatbuffers::schema::type_identifier<payload>();
...
TEST_CASE(sorted_entries_support_binary_search_lookup) {
    ...
    auto entries = eventide::serde::flatbuffers::schema::to_sorted_entries(input);
    ASSERT_EQ(entries.size(), 3U);
    EXPECT_EQ(entries[0].key, "alpha");
    EXPECT_EQ(entries[1].key, "mid");
    EXPECT_EQ(entries[2].key, "zeta");

    auto found = eventide::serde::flatbuffers::schema::bsearch_entry(entries, std::string("mid"));
