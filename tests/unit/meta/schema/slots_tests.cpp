...
TEST_CASE(simple_struct_slots) {
    ...
    STATIC_EXPECT_EQ(kind_of<typename slot0::raw_type>(), type_kind::int32);
    STATIC_EXPECT_EQ(kind_of<typename slot1::raw_type>(), type_kind::string);
    STATIC_EXPECT_EQ(kind_of<typename slot2::raw_type>(), type_kind::float32);

    // wire_type == raw_type for plain fields
    EXPECT_TYPE_EQ(typename slot0::raw_type, typename slot0::wire_type);
    EXPECT_TYPE_EQ(typename slot1::raw_type, typename slot1::wire_type);
    EXPECT_TYPE_EQ(typename slot2::raw_type, typename slot2::wire_type);

    // attrs is empty tuple for plain fields
    EXPECT_TYPE_EQ(typename slot0::attrs, std::tuple<>);
}
...
TEST_CASE(skip_and_flatten_slot_counts) {
    ...
    STATIC_EXPECT_EQ(kind_of<typename os0::raw_type>(), type_kind::int32);
    STATIC_EXPECT_EQ(kind_of<typename os1::raw_type>(), type_kind::int32);
    STATIC_EXPECT_EQ(kind_of<typename os2::raw_type>(), type_kind::int32);
    STATIC_EXPECT_EQ(kind_of<typename os3::raw_type>(), type_kind::int32);
}
...
TEST_CASE(behavior_wire_types) {
    ...
    using slot0 = type_list_element_t<0, slots>;
    EXPECT_TYPE_EQ(typename slot0::raw_type, std::optional<int>);
    EXPECT_TYPE_EQ(typename slot0::wire_type, std::optional<int>);
    STATIC_EXPECT_FALSE(std::is_same_v<typename slot0::attrs, std::tuple<>>);

    // Field 1 (as_str): raw=int, wire=std::string
    using slot1 = type_list_element_t<1, slots>;
    EXPECT_TYPE_EQ(typename slot1::raw_type, int);
    EXPECT_TYPE_EQ(typename slot1::wire_type, std::string);

    // Field 2 (plain): raw=wire=float, empty attrs
    using slot2 = type_list_element_t<2, slots>;
    STATIC_EXPECT_EQ(kind_of<typename slot2::raw_type>(), type_kind::float32);
    STATIC_EXPECT_EQ(kind_of<typename slot2::wire_type>(), type_kind::float32);
    EXPECT_TYPE_EQ(typename slot2::attrs, std::tuple<>);
}
...
TEST_CASE(enum_string_slot) {
    ...
    EXPECT_TYPE_EQ(typename slot0::raw_type, fx::Color);
    EXPECT_TYPE_EQ(typename slot0::wire_type, std::string_view);
}
...
TEST_CASE(with_wire_type_slot) {
    ...
    EXPECT_TYPE_EQ(typename slot0::raw_type, int);
    EXPECT_TYPE_EQ(typename slot0::wire_type, std::string);
}
...
TEST_CASE(tagged_variant_slot) {
    ...
    EXPECT_TYPE_EQ(typename slot0::raw_type, std::variant<int, std::string>);
    EXPECT_TYPE_EQ(typename slot0::wire_type, std::variant<int, std::string>);
