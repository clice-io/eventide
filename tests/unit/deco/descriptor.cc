...
struct DescOpt {
    ...
    DecoKVStyled(kota::deco::decl::KVStyle::Joined, names = {"-I", "--include"}; meta_var = "DIR";
...
TEST_CASE(from_deco_option_renders_usage_style_text) {
    ...
    EXPECT_TRUE(kota::deco::desc::from_deco_option(opt.verbose) == "-v|--verbose");
    EXPECT_TRUE(kota::deco::desc::from_deco_option(opt.output) == "-o|--output <FILE>");
    EXPECT_TRUE(kota::deco::desc::from_deco_option(opt.include_dir) == "-I<DIR>|--include=<DIR>");
    EXPECT_TRUE(kota::deco::desc::from_deco_option(opt.filter) ==
                "--filter <PATTERN>|--filter=<PATTERN>");
    EXPECT_TRUE(kota::deco::desc::from_deco_option(opt.tags) ==
                "--tags,<TAG>[,<TAG>...]|-T,<TAG>[,<TAG>...]");
    EXPECT_TRUE(kota::deco::desc::from_deco_option(opt.pair) == "--pair <VAL1> <VAL2>");
    EXPECT_TRUE(kota::deco::desc::from_deco_option(opt.input) == "<INPUT>");
...
TEST_CASE(from_deco_option_renders_help_style_text) {
    ...
    const auto verbose_help = kota::deco::desc::from_deco_option(opt.verbose, true);
    ...
    const auto output_help = kota::deco::desc::from_deco_option(opt.output, true);
    ...
    const auto filter_help = kota::deco::desc::from_deco_option(opt.filter, true);
    ...
    const auto input_help = kota::deco::desc::from_deco_option(opt.input, true);
    ...
    EXPECT_TRUE(kota::deco::desc::from_deco_option(vector_opt.inputs) == "<INPUT>...");
    const auto vector_input_help = kota::deco::desc::from_deco_option(vector_opt.inputs, true);
...
TEST_CASE(from_deco_option_uses_configured_help_layout_and_default_help) {
    ...
    const auto verbose_help = kota::deco::desc::from_deco_option(opt.verbose, true, {}, &config);
    EXPECT_TRUE(verbose_help.find("-v, --verbose") != std::string::npos);
...
TEST_CASE(from_deco_option_infers_enum_meta_var_for_vector_results) {
    ...
    EXPECT_TRUE(kota::deco::desc::from_deco_option(opt.values, false, "values") ==
                "--values,<alpha|beta|gamma>[,<alpha|beta|gamma>...]");
    EXPECT_TRUE(kota::deco::desc::from_deco_option(opt.pair, false, "pair") ==
                "--pair <alpha|beta|gamma> <alpha|beta|gamma>");
    EXPECT_TRUE(kota::deco::desc::from_deco_option(opt.inputs) == "<alpha|beta|gamma>...");
}
