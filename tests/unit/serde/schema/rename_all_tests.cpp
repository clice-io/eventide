...
struct CamelConfig {
    using field_rename = spelling::rename_policy::lower_camel;
};

struct PascalConfig {
    using field_rename = spelling::rename_policy::upper_camel;
};

struct UpperSnakeConfig {
    using field_rename = spelling::rename_policy::upper_snake;
};

struct LowerSnakeConfig {
    using field_rename = spelling::rename_policy::lower_snake;
};

struct IdentityConfig {
    using field_rename = spelling::rename_policy::identity;
};
...
struct AliasRenameAllStruct {
    annotation<int, schema::alias<"user_id">> id;
    float total_score;
};
...
TEST_CASE(rename_policies) {
    ...
    {
        constexpr auto& fields =
            virtual_schema<test_schema::RenameAllTarget, config::default_config>::fields;
        EXPECT_EQ(fields[0].name, "user_name");
        EXPECT_EQ(fields[1].name, "total_score");
        EXPECT_EQ(fields[2].name, "item_id");
    }
