...
using schema::field_info;
using schema::virtual_schema;
...
struct AnnotatedStruct {
    annotation<int, schema::rename<"id">> user_id;
    annotation<std::string, schema::skip> internal;
    float value;
};
...
struct AliasStruct {
    annotation<int, schema::alias<"user_id", "userId">> id;
    std::string name;
};
...
struct Outer {
    int x;
    annotation<Inner, schema::flatten> inner;
    int y;
};
...
struct Middle {
    int m;
    annotation<DeepInner, schema::flatten> deep;
};

struct DeepOuter {
    int head;
    annotation<Middle, schema::flatten> mid;
    int tail;
};
...
struct DefaultLiteralStruct {
    annotation<int, schema::default_value> with_default;
    annotation<std::string, schema::literal<"v1">> version;
    int plain;
};
...
TEST_CASE(flatten) {
    ...
    constexpr auto outer_inner_offset = refl::field_offset<test_schema::Outer>(1);
    constexpr auto inner_a_offset = refl::field_offset<test_schema::Inner>(0);
    constexpr auto inner_b_offset = refl::field_offset<test_schema::Inner>(1);
