...
using schema::kind_of;
using schema::virtual_schema;
using schema::type_list;
using schema::type_list_size_v;
using schema::type_list_element_t;
using schema::field_slot;
...
struct EnumStringStruct {
    annotation<color, behavior::enum_string<spelling::rename_policy::identity>> color_field;
    int count;
};
...
struct TaggedVariantStruct {
    annotation<std::variant<int, std::string>, schema::tagged<>> tv;
};
