...
constexpr std::string_view enum_to_string(E value) {
    ...
    if constexpr(enum_traits<E>::encoding == enum_encoding::string) {
        static_assert(
            requires { enum_traits<E>::mapping; },
...
struct Reflector<E> {
    using ReflType = std::conditional_t<refl::serde::enum_traits<E>::encoding ==
                                            refl::serde::enum_encoding::integer,
                                        std::underlying_type_t<E>,
                                        std::string>;

    static ReflType from(const E& value) {
        if constexpr(refl::serde::enum_traits<E>::encoding == refl::serde::enum_encoding::integer) {
