...
// Mirrors arena::detail::root_unboxed_v but gated by the backend's inline
// struct predicate — unboxed root requires the type *not* to be routed
...
// A root value read from a flatbuffer-style arena. Mirrors the code path in
// the flatbuffer deserializer's top-level decode_root_value but backend-agnostic.
template <typename Config, typename B, typename T>
    ...
    if constexpr(std::same_as<element_clean_t, std::byte>) {
        ...
        for(std::size_t i = 0; i < vec.size(); ++i) {
            ...
            if constexpr(!std::default_initializable<dec_t>) {
                ...
                dec_t element{};
                auto nested = vec[i];
                // root-boxed element: a single-field wrapper table with
                // the element at the first slot.
                auto sid_r = B::field_slot_id(0);
                if(!sid_r) {
                    return std::unexpected(sid_r.error());
                }
