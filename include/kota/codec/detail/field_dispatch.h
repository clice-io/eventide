...
constexpr auto serialize_struct_field(SerializeStruct& s_struct, Field field)
    ...
    if constexpr(!meta::annotated_type<field_t>) {
        ...
        // Schema: flatten — inline nested struct fields
            ...
            if constexpr(tuple_any_of_v<attrs_t, meta::is_rename_attr>) {
                using rename_attr = tuple_find_t<attrs_t, meta::is_rename_attr>;
            ...
            if constexpr(tuple_has_spec_v<attrs_t, meta::behavior::skip_if>) {
                ...
                if(meta::evaluate_skip_predicate<Pred>(value, true)) {
                    return std::expected<void, E>{};
                }
            }
...
constexpr auto deserialize_struct_field(DeserializeStruct& d_struct,
    ...
    if constexpr(!meta::annotated_type<field_t>) {
        ...
        // Schema: flatten — recurse into nested struct fields
            ...
            if constexpr(tuple_any_of_v<attrs_t, meta::is_alias_attr>) {
                if(!name_matched) {
