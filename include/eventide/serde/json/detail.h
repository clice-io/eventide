...
/// Shared array serializer for JSON backends.
/// `S` is the concrete Serializer type that provides `key()`, `end_array()`, etc.
template <typename S>
...
/// Shared object serializer for JSON backends.
/// `S` is the concrete Serializer type that provides `key()`, `end_object()`, etc.
template <typename S>
