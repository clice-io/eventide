...
public:
    std::string emit(const type_info* root) {
        ...
        // Build the root type schema inline (keys merged into top-level object)
        write_schema_fields(root);

        // Append $defs if any structs were collected
        if(!defs.empty()) {
            out += ',';
            key("$defs");
            out += '{';
...
private:
    ...
    // --- Schema builders returning self-contained JSON fragments ---

    /// Write a complete schema object for `ti` into `out`.
    ...
    /// Write schema fields (without outer braces) — used to merge into root object.
    void write_schema_fields(const type_info* ti) {
        ti = unwrap(ti);
        // For non-struct roots, wrap as a single schema object inline
        if(ti->kind != tk::structure) {
            // Capture schema into a temp, then strip outer braces and prepend comma
        ...
        // For struct root, inline the object schema fields
        auto* si = static_cast<const struct_type_info*>(ti);
        auto name = normalize_name(ti->type_name);
        // Mark visited so recursive refs still use $ref
        visited.insert(name);
    ...
    void ensure_struct_def(const type_info* ti, const std::string& name) {
        ...
        // Build the definition into a separate buffer
        auto saved = std::move(out);
        out.clear();
