...
- Default CLI runner: filter by `suite[.test]` with wildcards (`--test-filter=…`), `--verbose`, and `--parallel` for multi-process execution (`--parallel-workers=N` caps the worker count, 0 = all available cores).
